<select name="{{ $name }}@isset($isMultiple)[]@endisset" @isset($isMultiple) multiple="" @endisset class="{{ $classes ?? 'select2 form-control' }}" id="{{ $id ?? $name }}" data-toggle="cities_update" @isset($target) data-target="{{ $target }}" @endisset data-value="{{ $value }}" {{ $attributes ?? '' }}>
