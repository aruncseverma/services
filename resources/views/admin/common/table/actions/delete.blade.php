@php
$id = $id ?? '';
$btnId = $btnId ?? 'delete_' . $id;
$btnText = $btnText ?? '';
$btnCls = $btnCls ?? 'btn btn-danger btn-xs';
@endphp
<button type="button" id="{{ $btnId }}" class="{{ $btnCls }}" title="{{ __('Delete') }}" data-delete-id="{{ $id }}" data-delete-to="{{ $to }}">
    <i class="mdi mdi-delete"></i> {{ $btnText }}
</button>
