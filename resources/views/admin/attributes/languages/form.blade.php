    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                ...
                <form class="form es es-validation" action="{{ route('admin.attribute.languages.save') }}" method="POST">
                    ...
                    @foreach ($languages as $language)
                        @component('Admin::common.form.group', ['key' => 'descriptions.' . $language->code, 'labelClasses' => 'es-required', 'labelId' => 'descriptions_' . $language->code])
                            @slot('label')
                                @lang('Language Name') ({{ $language->name }}) <span class="text text-danger">*</span>
                            @endslot

                            @slot('input')
                                <input type="text" name="descriptions[{{ $language->code }}]" class="form-control @if (!$attribute->getKey() && $loop->first) es es-copy-text @else auto-value @endif" @if (!$attribute->getKey() && $loop->first) data-copy-text-target=".auto-value" @endif id="descriptions_{{ $language->code }}" placeholder="{{ __("Language Name in ({$language->name})")}}" value="{{ $attribute->getDescription($language->code)->content }}">
                            @endslot
                        @endcomponent
                    @endforeach
                    {{-- end description --}}
                    ...
                    <div class="form-actions pull-right">
                        ...
                        <a href="{{ route('admin.attributes.languages.manage') }}" class="btn btn-inverse">@lang('Cancel')</a>
                    </div>
