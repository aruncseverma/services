<div class="col-12">
    <div class="card">
        <div class="card-header" data-toggle="collapse" data-target="#aboutme">
            ...
            <span>@lang('About Myself')</span>
        </div>
        <div class="card-header-sub">
            @lang('Tell something about your member')
        </div>
        <div class="card-body collapse show" id="aboutme">
            ...
            <form action="{{ route('member_admin.profile.update_about') }}" method="POST" class="es es-validation" id="form_about">
                @csrf
                ...
                <div class="tab-content tabcontent-border m-b-20">
                    ...
                    <div class="tab-pane  @if ($loop->iteration == $showTab) active show @endif" id="tab-about-{{ $language->code }}" role="tabpanel">
                        <div class="p-20">
                            ...
                            <textarea name="about[{{ $language->code }}]" class="form-control" id="about-{{ $language->code }}" rows="3" placeholder="@lang('About yourself')">@if(empty(old()) || old('notify') == 'about'){{ $member->getDescription($language->code)->content }}@else{{ old("about.{$language->code}") }}@endif</textarea>
                            @include('MemberAdmin::common.form.error', ['key' => "about.{$language->code}"])
                        </div>
                    </div>
                    @endforeach
                </div>
